...
db_path = 'encrypted_color_data_ckks.db'
...
def encrypt(image_path, context):
    ...
    # 对每个通道进行加密
    encrypted_r = ts.ckks_vector(context, r_channel.flatten().tolist())
    encrypted_g = ts.ckks_vector(context, g_channel.flatten().tolist())
    encrypted_b = ts.ckks_vector(context, b_channel.flatten().tolist())
...
def decrypt(encrypted_r_blob, encrypted_g_blob, encrypted_b_blob, save_path, context):
    # 加载加密数据
    encrypted_r = ts.ckks_vector_from(context, encrypted_r_blob)
    encrypted_g = ts.ckks_vector_from(context, encrypted_g_blob)
    encrypted_b = ts.ckks_vector_from(context, encrypted_b_blob)
...
if __name__ == "__main__":
    print("CKKS start...")
    context_1 = ts.context(ts.SCHEME_TYPE.CKKS, poly_modulus_degree=8192, coeff_mod_bit_sizes=[60, 40, 40, 60])
    context_1.global_scale = 2**40
